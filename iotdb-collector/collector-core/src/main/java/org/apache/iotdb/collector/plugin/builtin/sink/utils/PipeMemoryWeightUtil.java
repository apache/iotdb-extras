...
public class PipeMemoryWeightUtil {
  ...
  private static Pair<Integer, Integer> calculateTabletRowCountAndMemoryBySize(
    ...
    int rowNumber =
        8
            * (PipeOptions.PIPE_DATA_STRUCTURE_TABLET_SIZE_IN_BYTES.value() - 100)
    ...
    rowNumber > PipeOptions.PIPE_DATA_STRUCTURE_TABLET_ROW_SIZE.value()) {
      // Bound the row number, the memory cost is rowSize * rowNumber
      return new Pair<>(
          PipeOptions.PIPE_DATA_STRUCTURE_TABLET_ROW_SIZE.value(),
          rowSize * PipeOptions.PIPE_DATA_STRUCTURE_TABLET_ROW_SIZE.value());
    } else {
      return new Pair<>(rowNumber, PipeOptions.PIPE_DATA_STRUCTURE_TABLET_SIZE_IN_BYTES.value());
    }
  }
