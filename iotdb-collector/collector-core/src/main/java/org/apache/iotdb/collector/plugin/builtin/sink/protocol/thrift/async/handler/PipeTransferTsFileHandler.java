...
package org.apache.iotdb.collector.plugin.builtin.sink.protocol.thrift.async.handler;
...
import org.apache.iotdb.collector.plugin.builtin.sink.payload.evolvable.request.PipeTransferTsFilePieceReq;
...
public class PipeTransferTsFileHandler extends PipeTransferTrackableHandler {
  ...
  private final List<PipeRawTabletInsertionEvent> events;
  ...
  public PipeTransferTsFileHandler(
    ...
    readFileBufferSize =
        (int)
            Math.min(
                PipeOptions.PIPE_CONNECTOR_READ_FILE_BUFFER_SIZE.value(),
                transferMod ? Math.max(tsFile.length(), modFile.length()) : tsFile.length());
    // memoryBlock =
    //     PipeDataNodeResourceManager.memory()
    //         .forceAllocateForTsFileWithRetry(
    //             PipeOptions.PIPE_CONNECTOR_READ_FILE_BUFFER_MEMORY_CONTROL.value()
    //                 ? readFileBufferSize
    //                 : 0);
      ...
      try {
        ...
        if (events.size() <= 1 || LOGGER.isDebugEnabled()) {
          LOGGER.info(
              ...
              tsFile,
              events.stream().map(PipeRawTabletInsertionEvent::getCommitterKey).collect(Collectors.toList()),
              events.stream().map(PipeRawTabletInsertionEvent::getCommitId).collect(Collectors.toList()),
              referenceCount);
        } else {
          LOGGER.info(
