...
package org.apache.iotdb.collector.plugin.builtin.sink.protocol.thrift.async;
...
import org.apache.iotdb.collector.plugin.builtin.sink.payload.evolvable.request.PipeTransferTabletRawReqV2;
import org.apache.iotdb.collector.plugin.builtin.sink.protocol.airgap.IoTDBConnector;
import org.apache.iotdb.collector.plugin.builtin.sink.protocol.thrift.async.handler.PipeTransferTabletBatchEventHandler;
import org.apache.iotdb.collector.plugin.builtin.sink.protocol.thrift.async.handler.PipeTransferTabletRawEventHandler;
import org.apache.iotdb.collector.plugin.builtin.sink.protocol.thrift.async.handler.PipeTransferTrackableHandler;
import org.apache.iotdb.collector.plugin.builtin.sink.protocol.thrift.async.handler.PipeTransferTsFileHandler;
import org.apache.iotdb.collector.plugin.builtin.sink.protocol.thrift.client.IoTDBDataNodeAsyncClientManager;
import org.apache.iotdb.collector.plugin.builtin.sink.protocol.thrift.client.async.AsyncPipeDataTransferServiceClient;
import org.apache.iotdb.collector.plugin.builtin.sink.protocol.thrift.sync.IoTDBDataRegionSyncConnector;
...
public class IoTDBDataRegionAsyncConnector extends IoTDBConnector {
  ...
  public void transfer(final TabletInsertionEvent tabletInsertionEvent) throws Exception {
    ...
    if (!(tabletInsertionEvent instanceof PipeRawTabletInsertionEvent)) {
      LOGGER.warn(
          "IoTDBThriftAsyncConnector only support PipeRawTabletInsertionEvent. "
              + "Current event: {}.",
  ...
  private void transferInBatchWithoutCheck(
    ...
    if (batch instanceof PipeTabletEventPlainBatch) {
      ...
      final List<PipeRawTabletInsertionEvent> events = tsFileBatch.deepCopyEvents();
  ...
  private void transferQueuedEventsIfNecessary() throws Exception {
    while (!retryEventQueue.isEmpty()) {
      synchronized (this) {
        ...
        if (peekedEvent instanceof PipeRawTabletInsertionEvent) {
        ...
        if (peekedEvent instanceof PipeRawTabletInsertionEvent) {
          ((PipeRawTabletInsertionEvent) peekedEvent)
  ...
  public void addFailureEventToRetryQueue(final Event event) {
    if (event instanceof PipeRawTabletInsertionEvent && ((PipeRawTabletInsertionEvent) event).isReleased()) {
      return;
    }

    if (isClosed.get()) {
      if (event instanceof PipeRawTabletInsertionEvent) {
        ((PipeRawTabletInsertionEvent) event).clearReferenceCount(IoTDBDataRegionAsyncConnector.class.getName());
      }
      return;
    }
  ...
  /**
   * Add failure {@link PipeRawTabletInsertionEvent}s to retry queue.
   *
   * @param events {@link PipeRawTabletInsertionEvent}s to retry
   */
  public void addFailureEventsToRetryQueue(final Iterable<PipeRawTabletInsertionEvent> events) {
  ...
  public synchronized void discardEventsOfPipe(final String pipeNameToDrop, final int regionId) {
    ...
    retryEventQueue.removeIf(
        event -> {
          if (event instanceof PipeRawTabletInsertionEvent
              && pipeNameToDrop.equals(((PipeRawTabletInsertionEvent) event).getPipeName())
              && regionId == ((PipeRawTabletInsertionEvent) event).getRegionId()) {
  ...
  public int getRetryEventCount(final String pipeName) {
    ...
    try {
      retryEventQueue.forEach(
          event -> {
            if (event instanceof PipeRawTabletInsertionEvent
                && pipeName.equals(((PipeRawTabletInsertionEvent) event).getPipeName())) {
