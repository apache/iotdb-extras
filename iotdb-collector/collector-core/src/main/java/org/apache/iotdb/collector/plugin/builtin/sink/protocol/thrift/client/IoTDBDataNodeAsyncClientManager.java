...
import org.apache.iotdb.collector.plugin.builtin.sink.protocol.thrift.client.factory.ClientPoolFactory;
...
public class IoTDBDataNodeAsyncClientManager extends IoTDBClientManager
  ...
  private boolean handshakeIfNecessary(
    ...
    try {
      ...
      params.put(
          PipeTransferHandshakeConstant.HANDSHAKE_KEY_CLUSTER_ID, PipeOptions.CLUSTER_ID.value());
      params.put(
      ...
      client.setTimeoutDynamically(PipeOptions.PIPE_CONNECTOR_HANDSHAKE_TIMEOUT_MS.value());
      ...
      while (!isHandshakeFinished.get()) {
        if (isClosed
            || System.currentTimeMillis() - startTime
                > PipeOptions.PIPE_CONNECTOR_HANDSHAKE_TIMEOUT_MS.value() * 2L) {
