...
public class MPPPublishHandler extends AbstractInterceptHandler {
  ...
  private final ConcurrentHashMap<String, ITableSession> clientIdToTableSessionMap =
      new ConcurrentHashMap<>();
  ...
  public MPPPublishHandler(MqttConfig config) {
    this.config = config;
    this.payloadFormat = config.getPayloadFormatManager().getPayloadFormat(config.getMqttPayloadFormatter());
  ...
  public void onConnect(InterceptConnectMessage msg) {
    ...
    if (useTableInsert) {
      clientIdToTableSessionMap.computeIfAbsent(msg.getClientID(), (cId) -> {
        ...
        sessionBuilder.nodeUrls(Collections.singletonList(config.getIotdbHost() + ":" + config.getIotdbPort()))
            .username(msg.getUsername()).password(new String(msg.getPassword()));
  ...
  public void onPublish(InterceptPublishMessage msg) {
    try {
      ...
      List<org.apache.iotdb.protocol.mqtt.Message> messages = payloadFormat.format(topic, payload);
      if (messages == null) {
        return;
      }
      ...
      for (Message message : messages) {
        ...
        if (useTableInsert) {
          insertTable((org.apache.iotdb.protocol.mqtt.TableMessage) message, session);
        } else {
          insertTree((org.apache.iotdb.protocol.mqtt.TreeMessage) message, session);
        }
      }
    } catch (Throwable t) {
