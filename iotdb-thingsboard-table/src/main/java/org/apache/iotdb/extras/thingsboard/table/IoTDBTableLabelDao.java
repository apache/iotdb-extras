 ...
 *
 * <p>This class is intentionally not annotated as a Spring bean. The optional label path is not
 * implemented here, so registering it now would expose an {@code iotdb.labels.enabled=true}
 * selector that ThingsBoard cannot bind to a working DAO.
 *
...
public class IoTDBTableLabelDao extends IoTDBTableBaseDao {
  ...
  // iotdb.labels.enabled=true with no working DAO behind it.
